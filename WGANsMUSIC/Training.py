...
def main():
    ...
    # Build 
    gan.discriminator.build(input_shape=(None, 128, 128, 1))
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for img_real in tqdm.tqdm(train_ds, position=0, leave=True):
            n_critic_count = 0
            step += 1
            current_batch_size = img_real.shape[0]
            # Train critic (discriminator)
            gan.WGAN_GP_train_d_step(img_real, batch_size=tf.constant(current_batch_size, dtype=tf.int64), step=tf.constant(step, dtype=tf.int64))
            n_critic_count += 1

            if n_critic_count >= N_CRITIC: 
                # Train generator
                gan.WGAN_GP_train_g_step(img_real, batch_size= tf.constant(current_batch_size, dtype=tf.int64), step=tf.constant(step, dtype=tf.int64))
                n_critic_count = 0
...
def log(train_summary_writer, gan, epoch):
    ...
    generated_imgs = tf.reshape(generated_imgs, shape=(num_generated_imgs, 128, 128, 1))
